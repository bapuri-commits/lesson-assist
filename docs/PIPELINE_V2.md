...
| **데이터 수집 + 가공** | school_sync | 크롤링, 정규화, 과목별 학습 컨텍스트 생성, PDF/PPT 다운로드 |
| **전사** | 다글로 (외부) | SRT/TXT 전사본 생성 |
| **패키징 + 노트** | lesson-assist v2 | school_sync 컨텍스트 + 다글로 전사본 → NotebookLM 패키지, Obsidian 노트 생성 |

> **핵심 원칙:** school_sync가 데이터의 수집과 가공을 전부 담당하고, lesson-assist는 이미 가공된 결과물을 받아서 패키징/포맷팅/전달만 수행한다.
...
**school_sync가 생성한** 과목별 학습 컨텍스트 파일을 읽는다. lesson-assist는 가공 로직을 갖지 않고, school_sync의 `output/context/{과목명}.md`를 그대로 사용한다.

**school_sync `context_export.py`가 생성하는 데이터:**
- 강의계획서: 주차별 토픽, 교수 정보, 교재, 강의 목표/개요
- 과제: 현재 활성 과제 목록, 마감일, 상태
- 공지사항: 해당 과목 관련 공지 (최근 2주)
- YAML frontmatter: 과목명, 생성 시각, 현재 주차/토픽

**제외 데이터 (school_sync 수준에서 필터링):**
- 성적, 출석 → 학습과 직접 관련 없음 (학사 관리는 ask.py 영역)
- 학교 행사, 장학금 → 학습 무관

> lesson-assist는 `school_sync/output/context/` 경로만 알면 되며, 내부 정규화 구조에 의존하지 않는다.
...
| 모듈 | 역할 |
|------|------|
| `packer.py` | Phase 2 오케스트레이션 — SRT 정제, 컨텍스트 읽기, 가이드 생성, 폴더 열기 |
...
```yaml
# school_sync 연동 (school_sync가 생성한 가공 데이터만 참조)
school_sync:
  root: "C:\\path\\to\\school_sync"
  context_dir: "output/context"       # 과목별 학습 컨텍스트 (school_sync가 생성)
  downloads_dir: "output/downloads"   # 수업자료 PDF/PPT 원본
